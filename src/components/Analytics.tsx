...
export default function Analytics({ quakes }: { quakes: Quake[] }) {
  const analysis = useMemo(() => {
    ...
    const bins = fmd(quakes.map((q) => q.mag), 0.1)
    ...
    const byYear = new Map<number, { eventos: number; energia: number; max: number }>()
  ...
  if (!analysis) return <Empty>Se necesitan al menos 30 eventos para el análisis estadístico.</Empty>
  const { gr, fmdChart, yearChart, depthChart, hourChart, scatter, cluster } = analysis
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          hint={gr ? `± ${gr.bErr.toFixed(2)} · ${interpretB(gr.b).label}` : undefined}
          accent="#38bdf8"
        />
        <Stat
        ...
        <Stat
          ...
          hint={gr ? `M ${magnitudeForReturnPeriod(gr, 100).toFixed(1)} cada 100 años` : undefined}
          accent="#facc15"
        />
      </div>
      ...
      >
        <div className="h-72">
          <ResponsiveContainer>
            <ComposedChart data={fmdChart} margin={{ top: 8, right: 12, bottom: 4, left: 0 }}>
              <CartesianGrid {...grid} />
              <XAxis dataKey="mag" {...axis} tickFormatter={(v: number) => v.toFixed(1)} />
              <YAxis scale="log" domain={['auto', 'auto']} allowDataOverflow {...axis} />
              <RTooltip
                contentStyle={tooltipStyle}
                formatter={(v: unknown, name: unknown) => [fmtNum(Number(v), 1), String(name)]}
                labelFormatter={(v: unknown) => `M ${Number(v).toFixed(1)}`}
              />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="incremental" name="Por bin" fill="#1e40af" opacity={0.65} />
              <Line
                type="monotone"
                dataKey="acumulado"
                name="Acumulado (N ≥ M)"
        ...
        <Note>
          El valor b describe la proporción entre sismos pequeños y grandes. Alrededor de 1 es lo
          normal en corteza continental; valores bajos indican mayor peso relativo de los sismos
          grandes. El ajuste solo es válido por encima de Mc.
        </Note>
      </Card>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Actividad por año" subtitle="Conteo de eventos y energía liberada">
          <div className="h-64">
            <ResponsiveContainer>
              <ComposedChart data={yearChart} margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
                <CartesianGrid {...grid} />
                ...
                <RTooltip
                  ...
                  formatter={(v: unknown, n: unknown) => [
                    n === 'Energía (PJ)' ? fmtNum(Number(v), 2) : fmtNum(Number(v), 0),
                    String(n),
                  ]}
                />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar yAxisId="l" dataKey="eventos" name="Eventos" fill="#1d4ed8" />
                <Line
                  yAxisId="r"
                  type="monotone"
                  dataKey="energia"
          ...
          <Note>
            La energía crece 32 veces por cada unidad de magnitud: un solo M8 libera más que todos
            los M6 de una década juntos.
          </Note>
        </Card>
        ...
        <Card title="Distribución de profundidad" subtitle="Bins de 25 km">
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={depthChart} margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
                <CartesianGrid {...grid} />
                <XAxis dataKey="km" {...axis} unit=" km" />
                <YAxis {...axis} />
                <RTooltip
                  contentStyle={tooltipStyle}
                  labelFormatter={(v: unknown) => `${Number(v)}–${Number(v) + 25} km`}
                />
                <Bar dataKey="eventos" name="Eventos">
                  {depthChart.map((d) => (
                    <Cell
                      key={d.km}
                      fill={d.km < 70 ? '#f87171' : d.km < 300 ? '#facc15' : '#60a5fa'}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <Note>
            Rojo = superficial (&lt; 70 km, el más dañino), amarillo = intermedio, azul = profundo
            dentro de la placa subducida.
          </Note>
        </Card>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <ScatterChart margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
                <CartesianGrid {...grid} />
                <XAxis type="number" dataKey="depth" name="Profundidad" unit=" km" {...axis} />
                <YAxis type="number" dataKey="mag" name="Magnitud" domain={['auto', 'auto']} {...axis} />
        ...
        >
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={hourChart} margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
                <CartesianGrid {...grid} />
                <XAxis dataKey="hora" {...axis} />
                <YAxis {...axis} />
                <RTooltip contentStyle={tooltipStyle} labelFormatter={(v: unknown) => `${Number(v)}:00 UTC`} />
                <Bar dataKey="eventos" name="Eventos" fill="#0891b2" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <Note>
            Un reparto plano confirma que no existe "hora sísmica": la sensación contraria viene de
            que de noche se sienten más.
          </Note>
        </Card>
      </div>
            ...
            .map((q) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm text-slate-200">{q.place}</span>
                  <span className="block text-[11px] text-slate-500">
                    {new Date(q.time).getUTCFullYear()} · {q.depth.toFixed(0)} km de profundidad
                  </span>
                </span>
              </div>
            ))}
