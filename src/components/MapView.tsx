...
function HeatLayer({ quakes, enabled }: { quakes: Quake[]; enabled: boolean }) {
  ...
  useEffect(() => {
    ...
    const points = quakes.map(
      (q) =>
        [q.lat, q.lon, Math.min(1, 10 ** (q.mag - 6))] as [
          number,
          number,
          number,
        ],
...
export default function MapView({
  ...
  const { ordered, hidden } = useMemo(() => {
    ...
    return {
      ordered: byMag.slice(byMag.length - MAX_MARKERS),
      hidden: byMag.length - MAX_MARKERS,
    };
  }, [quakes]);
  const newest = useMemo(
    () =>
      quakes.reduce<Quake | null>(
        (mx, q) => (!mx || q.time > mx.time ? q : mx),
        null,
      ),
    [quakes],
  );
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-ink-800 ${className}`}
    >
      <MapContainer
        center={[15, 0]}
        zoom={2}
          ...
          ALL_REGIONS.map((r) => {
            ...
            return (
              <Rectangle
                ...
                eventHandlers={{
                  click: () => onSelectRegion?.(active ? null : r.id),
                }}
              >
                <Tooltip sticky>
        ...
        {!showHeat &&
          ordered.map((q) => (
            <CircleMarker
              ...
              pathOptions={{
                color:
                  colorBy === "mag" ? magColor(q.mag) : depthColor(q.depth),
            ...
            >
              <Popup>
                <div className="space-y-1">
                  ...
                  <div className="text-slate-400">
                    Profundidad {q.depth.toFixed(0)} km · {q.lat.toFixed(2)},{" "}
                    {q.lon.toFixed(2)}
                  </div>
                  {q.tsunami && (
                    <div className="text-amber-300">Con aviso de tsunami</div>
                  )}
                  <a
      ...
      {hidden > 0 && (
        <div className="pointer-events-none absolute bottom-2 left-2 z-[1000] rounded-lg border border-ink-700 bg-ink-950/85 px-2.5 py-1.5 text-[11px] text-slate-400">
          ...
          · {hidden.toLocaleString("es-CO")} ocultos. Sube la magnitud mínima o
          enfoca una zona.
        </div>
      )}
    </div>
