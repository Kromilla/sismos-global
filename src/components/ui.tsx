...
export function Stat({
  ...
  return (
    <div className="rounded-xl border border-ink-800 bg-ink-900/60 px-3 py-2.5">
      ...
      <div className="mt-1 font-mono text-xl leading-tight" style={accent ? { color: accent } : undefined}>
        {value}
      </div>
...
export function Note({ children, tone = 'info' }: { children: ReactNode; tone?: 'info' | 'warn' }) {
  ...
  return (
    <p className={`rounded-xl border px-3 py-2 text-xs leading-relaxed ${styles}`}>{children}</p>
  )
}
