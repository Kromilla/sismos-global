...
function urlFor(source: string, id: string): string {
  switch (source) {
    case "SGC":
      return "https://bdrsnc.sgc.gov.co/paginas1/catalogo/index.php";
    case "EMSC":
      return `https://www.seismicportal.eu/eventdetails.html?unid=${id.replace("EMSC:", "")}`;
    case "GEONET":
      return `https://www.geonet.org.nz/earthquake/${id.replace("GEONET:", "")}`;
    case "GA":
      return `https://earthquakes.ga.gov.au/event/${id.replace("GA:", "")}`;
    case "INGV":
      return `https://terremoti.ingv.it/event/${id.replace("INGV:", "")}`;
    case "GFZ":
      return `https://geofon.gfz-potsdam.de/eqinfo/event.php?id=${id.replace("GFZ:", "")}`;
    case "IPGP":
      return `http://ws.ipgp.fr/fdsnws/event/1/query?eventid=${id.replace("IPGP:", "")}&format=text`;
    case "SSN":
      return `http://www2.ssn.unam.mx:8080/catalogo/`;
    default:
      return `https://earthquake.usgs.gov/earthquakes/eventpage/${id.replace("USGS:", "")}`;
  }
}
...
export async function readCache(key: string): Promise<Quake[] | null> {
  const entry = await tx<CacheEntry>(
    "readonly",
    (store) => store.get(key) as IDBRequest<CacheEntry>,
  );
