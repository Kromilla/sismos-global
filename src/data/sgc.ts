...
function toQuake(a: SgcAttributes): Quake | null {
  ...
  if (mag == null || lat == null || lon == null || !Number.isFinite(stored))
    return null;
...
export async function fetchSgcCatalog(
  ...
  const q: CatalogQuery = {
    ...query,
    minMag: Math.max(query.minMag, SGC_MIN_MAG),
  };
  ...
  while (stack.length) {
    ...
    const n =
      out.length === 0 && stack.length === 0
        ? total
        : await countWindow(q, from, to, signal);
    if (n === 0) continue;
...
export async function loadSgcCatalog(
  ...
  const { quakes, truncatedWindows } = await fetchSgcCatalog(
    query,
    signal,
    onProgress,
  );
