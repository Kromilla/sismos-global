...
export const FDSN_NETWORKS: Record<string, FdsnNetwork> = {
  ...
  EMSC: {
    ...
    source: 'EMSC',
    eventUrl: (id) => `https://www.seismicportal.eu/eventdetails.html?unid=${id}`,
  },
  GEONET: {
    id: 'GEONET',
...
async function fetchJson(url: string, signal?: AbortSignal): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`${url.split('/')[2]} respondió ${res.status} ${res.statusText}`)
  return res.json()
}

async function countFdsn(net: FdsnNetwork, q: CatalogQuery, signal?: AbortSignal): Promise<number> {
  try {
...
async function fetchWindow(
  ...
  return (data.features ?? []).map((f) => toQuake(f, net)).filter((x): x is Quake => x !== null)
}
