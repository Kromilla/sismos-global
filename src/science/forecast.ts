...
export function buildRegionForecast(
  ...
  if (gr && grBackground) {
    const bgPerDay =
      grBackground.n / Math.max(grBackground.years * YEAR_DAYS, 1);
  ...
  const sequences = findSequences(quakes, {
    minMain: 5.3,
    lookbackDays: 120,
    now,
  });
...
export function backtest(
  ...
  const rows = mags.map((m) => {
    ...
    return {
      ...
      expected,
      observed,
      pAtLeastObserved: clamp(1 - cdf, 0, 1),
    };
  });
