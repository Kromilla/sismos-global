...
export function withObservations(
  quakes: Quake[],
...
export function scoreIpe(
  ...
  const within1 =
    residuals.filter((r) => Math.abs(r.residual) <= 1).length /
    residuals.length;
 ...
 */
export function biasByMagnitude(
  score: IpeScore,
  step = 0.5,
): ResidualByDistanceBin[] {
