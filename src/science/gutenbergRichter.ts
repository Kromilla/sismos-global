...
export function fmd(mags: number[], dM = 0.1): FmdBin[] {
  ...
  for (let i = 0; i < nBins; i++) {
    bins.push({ mag: +(min + i * dM).toFixed(2), count: counts[i], cumulative: cum })
    cum -= counts[i]
  }
  return bins
}
 ...
 */
export function interpretB(b: number): { label: string; tone: 'alto' | 'normal' | 'bajo' } {
  if (b < 0.8) return { label: 'b bajo: mayor peso relativo de sismos grandes', tone: 'bajo' }
  if (b > 1.2) return { label: 'b alto: dominan los sismos pequeños (enjambre/volcánico)', tone: 'alto' }
