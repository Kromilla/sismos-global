...
export function sitePsha(
  ...
  const magBins = Array.from({ length: nBins }, (_, i) => {
    const m1 = mc + i * dM
    const m2 = m1 + dM
    return { mag: m1 + dM / 2, weight: (10 ** (-b * m1) - 10 ** (-b * m2)) / norm }
  })
  ...
  for (const s of sources) {
    ...
    if (!historicMax || mmi > historicMax.mmi) historicMax = { mmi, quake: s.q }
  }
...
export function scenarioShaking(
  ...
  return cities
    .map((city) => {
      ...
      const mmi = ipe.mmi(scenario.mag, hypocentralKm(dist, scenario.depth)) + bias
      return { city, distKm: dist, mmi, pgaG: mmiToPga(mmi) / 980.665, popK: city.popK }
    })
    .filter((r) => r.mmi >= 2)
