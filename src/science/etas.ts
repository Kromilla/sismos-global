...
export function calibrateEtas(
  ...
  return { mu: opts.backgroundRatePerDay, K0, alpha, p, c, mc: opts.mc, branching }
}

/** Tasa instantánea de eventos M >= Mc por día, en el instante `at`. */
export function etasRate(params: EtasParams, quakes: Quake[], at: number): number {
  let rate = params.mu
  for (const q of quakes) {
    if (q.time >= at || q.mag < params.mc) continue
    const dt = (at - q.time) / DAY_MS
    rate += params.K0 * 10 ** (params.alpha * (q.mag - params.mc)) * (dt + params.c) ** -params.p
  }
  return rate
}
...
export function scaleToMagnitude(expectedAtMc: number, b: number, mc: number, m: number): number {
...
export function etasForecast(
  ...
  for (const h of horizons) {
    ...
    for (const m of mags) {
      ...
      out.push({ horizonDays: h, mag: m, expected, probability: poissonAtLeastOne(expected) })
    }
  }
  return out
}
