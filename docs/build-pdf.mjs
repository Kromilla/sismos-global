...
async function inlineFonts() {
  ...
  for (const variant of FONT_VARIANTS) {
    ...
    for (const fontUrl of [...new Set(css.match(/https:[^)]+\.woff2/g) ?? [])]) {
  ...
  console.log(`  fuentes: ${files} archivos woff2 estáticos, ${(bytes / 1024).toFixed(0)} KB`)
  return blocks.join('\n')
}
...
body = body.replace(/<canvas data-seed="(\d+)"><\/canvas>/g, (_, seed) => traceSvg(Number(seed)))
